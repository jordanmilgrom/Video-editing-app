...
ANALYZE_SCENE = (
    ...
    "(inline image + sidecar JSON with schema).\n"
    ...
    "reading again. `index_project` inlines it too, so a fresh chat "
    "surveying a shoot sees every clip's structured description in one "
    "call.\n\n"
    ...
    "raw video.\n\n"
    "Sample_hz controls motion resolution (default 2.0 = 500ms). "
    "num_frames controls contact-sheet density (default 25 = one frame "
    "every ~4% of clip duration). Bump num_frames to 36 for long clips "
    "where 4% granularity misses events. Synchronous, ~3-5s per minute "
    "of source (mostly motion analysis + one ffmpeg pass)."
)
...
SAVE_SCENE_ANALYSIS = (
    ...
    "auto-filled by the server if omitted — the agent only has to author "
    "the semantic fields.\n\n"
...
READ_SCENE_ANALYSIS = (
    ...
    "`ok=False, error='not_a_file'` if none exists yet. Filesystem read "
    "only — no vision, no motion recompute. Sub-millisecond."
)

SEARCH_SCENES = (
    "Case-insensitive substring search across every cached SceneAnalysis "
    "in the active cache dir. Matches against one_line, usability_verdict, "
    "color_palette, tags, and every shot's subject / composition / "
    "color_mood / quality / notable_events.\n\n"
    "Use for high-level content search once analyses are cached: "
    "'find clips with worker' returns matching clips; 'find shots with "
    "camera bump' surfaces bad takes; 'find blooper' finds flagged "
    "clips. Beats calling analyze_scene 30 times per session.\n\n"
    "Optional `folder_path` scopes the search to analyses whose source "
    "video lives under that folder. Results sorted by match count desc."
)
