...
def test_transcribe_runs_and_caches(tmp_path: Path, tiny_wav: Path, monkeypatch) -> None:
    ...
    def fake_whisper(wav_path: Path) -> dict:
        calls["n"] += 1
    ...
    cached_file = cache_dir / "transcripts" / f"{transcribe.cache_key(tiny_wav)}.json"
...
def test_transcribe_folder_skips_unsupported(tmp_path: Path, tiny_wav: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(transcribe, "_run_whisper", lambda p: FAKE_WHISPER_OUTPUT)
